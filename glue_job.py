...
carrier_lkp = spark.read.option("header", True) \
    ...
    .select(col("Code").alias("OP_CARRIER"),
            col("Airline Name").alias("carrier_full_name"))

# ======================================================
# AIRLINE GOLD (Operational Health)
# ======================================================
airline_gold = (
    customers_gold
    ...
    .withColumn("journey_friction_index",
    ...
    .withColumn("airport_bottleneck_score",
        (coalesce(col("avg_taxi_out"),lit(0)) +
         coalesce(col("avg_taxi_in"),lit(0))) /
